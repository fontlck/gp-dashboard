...
export async function GET(req: Request) {
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`
      DO $$ BEGIN
        CREATE TYPE "UploadStatus" AS ENUM ('PROCESSING', 'COMPLETED', 'FAILED', 'DUPLICATE');
      EXCEPTION WHEN duplicate_object THEN null;
      END $$;
    `)
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "Branch" (
        ...
        CONSTRAINT "Branch_partnerId_fkey" FOREIGN KEY ("partnerId") REFERENCES "Partner"("id") ON DELETE CASCADE ON UPDATE CASCADE
      );
      CREATE UNIQUE INDEX IF NOT EXISTS "Branch_code_key" ON "Branch"("code");
      CREATE INDEX IF NOT EXISTS "Branch_partnerId_idx" ON "Branch"("partnerId");
      CREATE INDEX IF NOT EXISTS "Branch_code_idx" ON "Branch"("code");
    `)
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "CsvUpload" (
        ...
        "totalGrossSales" DOUBLE PRECISION NOT NULL,
        "status" "UploadStatus" NOT NULL DEFAULT 'PROCESSING',
        "errorMessage" TEXT,
        "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT "CsvUpload_pkey" PRIMARY KEY ("id"),
        CONSTRAINT "CsvUpload_branchId_fkey" FOREIGN KEY ("branchId") REFERENCES "Branch"("id") ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT "CsvUpload_uploadedById_fkey" FOREIGN KEY ("uploadedById") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE
      );
      CREATE UNIQUE INDEX IF NOT EXISTS "CsvUpload_fileHash_key" ON "CsvUpload"("fileHash");
      CREATE INDEX IF NOT EXISTS "CsvUpload_branchId_idx" ON "CsvUpload"("branchId");
      CREATE INDEX IF NOT EXISTS "CsvUpload_uploadedById_idx" ON "CsvUpload"("uploadedById");
      CREATE INDEX IF NOT EXISTS "CsvUpload_status_idx" ON "CsvUpload"("status");
      CREATE INDEX IF NOT EXISTS "CsvUpload_createdAt_idx" ON "CsvUpload"("createdAt");
    `)
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "SalesRecord" (
        ...
        "netGP" DOUBLE PRECISION NOT NULL,
        ...
        CONSTRAINT "SalesRecord_csvUploadId_fkey" FOREIGN KEY ("csvUploadId") REFERENCES "CsvUpload"("id") ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT "SalesRecord_branchId_fkey" FOREIGN KEY ("branchId") REFERENCES "Branch"("id") ON DELETE CASCADE ON UPDATE CASCADE
      );
      CREATE INDEX IF NOT EXISTS "SalesRecord_date_idx" ON "SalesRecord"("date");
      CREATE INDEX IF NOT EXISTS "SalesRecord_branchId_idx" ON "SalesRecord"("branchId");
      CREATE INDEX IF NOT EXISTS "SalesRecord_csvUploadId_idx" ON "SalesRecord"("csvUploadId");
    `)
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        branches: [branch1.code, branch2.code],
      }
    })
  } catch (error: any) {
